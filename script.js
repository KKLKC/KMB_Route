...
searchBtn.addEventListener("click", async () => {
    const searchValue = searchbox.value.toUpperCase().replace(/\s+/g, "");
...
try {
    ...
    const routeChecked = routes.filter(route => route.route.replace(/\s+/g, "") === searchValue);

    if (routeChecked.length === 0) {
    showAlert("Bus route not found. Please try again.");
    return;
    }
